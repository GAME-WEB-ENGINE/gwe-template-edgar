...
class Room extends GWE.GfxDrawable {
  ...
  $screenFadeIn(delay, ms, timingFunction) {
    this.scriptMachine.setEnabled(false);
    this.uiManager.fadeIn(delay, ms, timingFunction, () => this.scriptMachine.setEnabled(true));
  }

  $screenFadeOut(delay, ms, timingFunction) {
    this.scriptMachine.setEnabled(false);
    this.uiManager.fadeOut(delay, ms, timingFunction, () => this.scriptMachine.setEnabled(true));
  }

  $sysWaitPad() {
    this.scriptMachine.setEnabled(false);
    Base.subscribeOnce(this.inputManager, this, 'E_KEYDOWN_ONCE', (data) => {
      if (data.key == KEY_ENTER) this.scriptMachine.setEnabled(true);
    });
  }
  ...
  $sysGotoIf(varloc, cond, value, jumpto) {
    let variant = this.player.getVariant(varloc);
    if ((cond == 'not equal' && variant != value) || (cond == 'equal' && variant == value) || (cond == 'is less than' && variant < value) || (cond == 'is greater than' && variant > value)) {
      return jumpto;
    }
  }

  $sysExecIf(varloc, cond, value, cmd = { CommandName, CommandArgs }) {
    let variant = this.player.getVariant(varloc);
    if ((cond == 'not equal' && variant != value) || (cond == 'equal' && variant == value) || (cond == 'is less than' && variant < value) || (cond == 'is greater than' && variant > value)) {
